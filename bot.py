...
_PROXY_ADD_STATE: dict[int, float] = {}     # admin is typing a single proxy line (uid → ts)
_PROXY_SOURCE_STATE: dict[int, float] = {}  # admin is typing a source URL to import (uid → ts)
...
# Maps uid → state dict with keys: step, netflix_id, old_pw, new_pw, ts
_CHANGEPW_STATE: dict[int, dict] = {}
# Abandoned Change-Password sessions expire after 15 minutes so they can
# never hijack a later normal cookie check.
_CHANGEPW_TTL_SEC = 15 * 60
...
def _row(emoji: str, label: str, value: str, label_w: int = 17) -> str:
    """
    Flush-left message row — every line starts at column 0 (no leading gap),
    and labels are padded so the '»' column lines up into straight lines.
    """
    return f"{emoji} {label.ljust(label_w)} » {value}"
...
def format_result_basic(result: dict, index: int = 1, total: int = 1, source: str = "") -> str:
    ...
    if name:
        lines.append(f"• {'Name:'.ljust(9)} {name}")
    lines.append(f"• {'Email:'.ljust(9)} <code>{email}</code>")
    if password:
        lines.append(f"• {'Password:'.ljust(9)} <code>{password}</code>")
    if phone:
        lines.append(f"• {'Phone:'.ljust(9)} {phone}")
    lines.append("")
    ...
    lines.append(f"• {'Country:'.ljust(9)} {country_disp}")
    lines.append(f"• {'Plan:'.ljust(9)} {plan or 'Unknown'}")
    lines.append(f"• {'Quality:'.ljust(9)} {quality}  ·  {streams} screens")
    lines.append(f"• {'Price:'.ljust(9)} {price}")
    lines.append(f"• {'Billing:'.ljust(9)} {billing}")
    if card_line and card_line not in ("Unknown", ""):
        lines.append(f"• {'Payment:'.ljust(9)} {card_line}")
    lines.append("")
...
# Matches separator lines: ──────────────── or ════════ or --------- or ———— (8+ chars)
_HIT_SEPARATOR_RE  = re.compile(r'(?m)^[\u2500\u2550\u2014\u2013\-=─═]{8,}\s*$')
# Matches LABELED separator/header lines where separator chars surround a short
# tag with a number, e.g.  "================== HIT #1 ==================="
# or  "---- ACCOUNT 5 ----"  /  "**** COOKIE 12 ****".  These do NOT match the
# bare-separator regex above (text breaks the char run), which previously made
# whole hit-dump files collapse into ONE giant "account".
_HIT_LABELED_SEP_RE = re.compile(
    r'(?im)^[^A-Za-z0-9\n]{3,}?\s*'
    r'(?:HIT|ACCOUNT|ACC|ENTRY|RESULT|COOKIE|USER|LOGIN)\s*#?\s*\d+'
    r'\s*[^A-Za-z0-9\n]*$'
)
# Bare "SecureNetflixId=" continuation lines need no special handling: the
# per-cookie slicer keeps every line AFTER a block's cookie line inside that
# same block, so continuation lines always stay with their account.


def _split_hit_dump_per_cookie(text: str) -> list[str]:
    """
    Last-resort splitter for hit dumps that have NO separator lines at all:
    slice the text into blocks anchored at each 'Cookie:' label line, so every
    block contains exactly one account cookie (plus whatever follows it up to
    the next cookie line).  Returns [] when there is fewer than 2 cookie lines
    (i.e. the text is a single ordinary account — nothing to split).
    """
    try:
        anchors = []
        for m in _HIT_BLOCK_COOKIE_RE.finditer(text):
            anchors.append(text.rfind("\n", 0, m.start()) + 1)   # line start
        if len(anchors) < 2:
            return []
        blocks = []
...
def _split_hit_dump(text: str) -> list[str]:
    """
    Split a hit-dump file (blocks carrying '• Cookie: ...' lines) into
    per-account blocks.  Strategy, most-to-least conservative:
      1. bare separator lines      (──────── / ════ / ------ / ————)
      2. labeled header lines      (==== HIT #1 ====)
      3. slice per 'Cookie:' line  (no separators at all)
    A split result is only adopted when it actually separates accounts
    (>1 cookie-bearing block) — single-account texts fall through unchanged.
    """
    def _n_cookies(block: str) -> int:
        # NOTE: 'SecureNetflixId=' contains 'NetflixId=' as a substring, so
        # counting only NetflixId= is sufficient for both.
        return block.count("NetflixId=")

    cookie_blocks = [p.strip() for p in
                     ([text] if not _HIT_SEPARATOR_RE.search(text)
                      else _HIT_SEPARATOR_RE.split(text))
                     if p.strip() and _HIT_BLOCK_COOKIE_RE.search(p)]

    # Bare separators worked and produced multiple accounts
    if len(cookie_blocks) > 1:
        return cookie_blocks

    # Everything collapsed into ONE giant block holding several cookies —
    # the dump uses labeled headers (or no headers).  Retry:
    if len(cookie_blocks) == 1 and _n_cookies(cookie_blocks[0]) > 1:
        lab_blocks = [p.strip() for p in _HIT_LABELED_SEP_RE.split(text)
                      if p.strip() and _HIT_BLOCK_COOKIE_RE.search(p)]
        if len(lab_blocks) > 1:
            return lab_blocks
        sliced = _split_hit_dump_per_cookie(text)
        if len(sliced) > 1:
            return sliced

    # No bare separators at all — try labeled headers, then per-cookie slice.
    ...
    return cookie_blocks   # single ordinary block (or original behaviour)
...
def split_cookies_from_text(text: str) -> list[str]:
    try:
        ...
        # Handles: bare separators, labeled headers (==== HIT #1 ====) and
        # separator-free dumps.  Single-cookie files keep the old path.
        if _HIT_BLOCK_COOKIE_RE.search(text) and (_HIT_SEPARATOR_RE.search(text)
                                                  or _HIT_LABELED_SEP_RE.search(text)):
            valid = _split_hit_dump(text)
            if valid:
                return valid
...
_ZIP_ENTRY_LIMIT = 2000   # max files read from a single archive (ZIP or RAR)

def _archive_kind(fname: str, mime: str) -> str:
    """Return 'zip', 'rar' or '' based on filename extension / MIME type."""
    f = (fname or "").lower()
    m = (mime or "").lower()
    if f.endswith(".zip") or m in ("application/zip", "application/x-zip-compressed"):
        return "zip"
    if f.endswith(".rar") or m in RAR_MIME_TYPES:
        return "rar"
    return ""
...
def read_cookie_texts_from_rar(rar_path: str) -> list[tuple[str, str]]:
    """
    Extract cookie text files from a RAR archive (RAR4 & RAR5 via rarfile +
    the unrar system tool).  Mirrors read_cookie_texts_from_zip: entry cap,
    binary skip, bad-entry skip.
    Raises rarfile.Error subclasses for corrupt/invalid archives so the
    caller can give the user a specific error message.
    """
    import rarfile  # local import — needs the unrar system tool at runtime
...
def _score_account(result: dict) -> tuple:
    ...
    Criteria (priority order):
      1. ACTIVE first — an on-hold account can NEVER outrank an active one,
         no matter how good its plan looks on paper
      2. Plan tier  (Premium > Standard > Basic > Mobile > unknown)
      3. No account issues
    ...
    not_hold  = 0 if (result.get("is_on_hold")
                      or result.get("status") == "on_hold") else 1
    ...
    return (not_hold, plan_score, no_issues, billing_days, member_days)
...
async def send_hits_zip(update: Update, hits: list[tuple[dict, str, str]]) -> None:
    ...
    # ── Categorise ────────────────────────────────────────────────────────
    # ONLY active accounts count as Premium/Normal hits.  An on-hold account
    # is NOT a hit (payment issue) — it gets its own folder so a held
    # premium-plan account can never inflate the "Premium Hits" numbers.
    premium = [(r, s, w) for r, s, w in deduped
               if r.get("status") != "on_hold"
               and "premium" in (r.get("plan_name") or "").lower()]
    normal  = [(r, s, w) for r, s, w in deduped
               if r.get("status") != "on_hold"
               and "premium" not in (r.get("plan_name") or "").lower()]
    onhold  = [(r, s, w) for r, s, w in deduped
               if r.get("status") == "on_hold"]
    ...
    def _account_file(i: int, total: int, result: dict, source: str) -> str:
        ...
        lines = [
            sep,
            (f"  NETFLIX ON-HOLD  #{i}/{total}   —   {plan.upper()}  ⏸ NOT ACTIVE"
             if status == "on_hold"
             else f"  NETFLIX HIT  #{i}/{total}   —   {plan.upper()}"),
            sep,
            "",
    ...
    try:
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            # Summary file
            active_count = len(premium) + len(normal)
            W = 52
            summary_lines = [
                "╔" + "═" * W + "╗",
                ...
                "╠" + "═" * W + "╣",
                f"║{'':^{W}}║",
                f"║  Total Found    :  {total_hits:<{W-20}}║",
                f"║  Active Hits    :  {active_count:<{W-20}}║",
                f"║  Premium Hits   :  {len(premium):<{W-20}}║",
                f"║  Normal Hits    :  {len(normal):<{W-20}}║",
                f"║  On Hold        :  {len(onhold):<{W-20}}║",
                f"║{'':^{W}}║",
            ]
            if dupes_removed > 0:
            ...
            # On Hold folder — held accounts are NOT hits, kept separate so
            # premium-plan holds never mix into the hit folders
            for i, (result, src, _) in enumerate(onhold, 1):
                email   = result.get("email") or f"account_{i}"
                safe    = re.sub(r'[^\w@._-]', '_', email)[:35]
                plan    = re.sub(r'[^\w ]', '', result.get("plan_name") or "Unknown")[:20].strip()
                c_flag  = _flag(result.get("country") or "")
                flag_pre = f"{c_flag}_" if c_flag else ""
                fname   = f"On Hold/{i:02d}_{flag_pre}{safe}_{plan}.txt"
                zf.writestr(fname, _account_file(i, len(onhold), result, src))
    ...
    caption_parts = [
        ...
        "",
        _row("🌟", "Premium Hits", f"<b>{len(premium)}</b>"),
        _row("✅", "Normal Hits", f"<b>{len(normal)}</b>"),
    ]
    if onhold:
        caption_parts.append(
            _row("⏸️", "On Hold", f"<b>{len(onhold)}</b> (not hits)"))
    caption_parts.append(
        _row("📊", "Total", f"<b>{total_hits}</b>"))
    if dupes_removed > 0:
        caption_parts.append(_row("♻️", "Dupes removed", f"<b>{dupes_removed}</b>"))
    caption_parts += [
        "",
        "📁 <b>ZIP structure:</b>",
        "<code>Premium Hits/</code> — Active premium accounts",
        "<code>Normal Hits/</code> — Active standard / basic files",
    ]
    if onhold:
        caption_parts.append(
            "<code>On Hold/</code> — Payment-held (not hits)")
    caption_parts += [
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "• <code>.zip</code> / <code>.rar</code> — Multiple files at once\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "<b>4. ZIP / RAR (.zip / .rar)</b>\n"
        "Drop an archive — each <code>.txt</code> / <code>.json</code> inside = 1 account.\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "📦 <b>Bulk mode</b>\n"
...
async def changepw_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    _CHANGEPW_STATE[uid] = {"step": "netflix_id", "ts": time.time()}
    ...
    await update.message.reply_text(
        ...
        "Step 1 of 3 — Send the account's <b>cookie</b>:\n"
        "• Paste the NetflixId value or the full cookie text\n"
        "• Or send ONE <code>.txt</code> / <code>.json</code> / <code>.zip</code> / "
        "<code>.rar</code> file holding a single account cookie\n\n"
        "<i>Nothing except the cookie is accepted in this mode.</i>",
        parse_mode=ParseMode.HTML,
    )
...
def _changepw_expired(uid: int) -> bool:
    """
    True if the user's Change-Password session is older than the TTL
    (the stale state is popped so normal cookie handling resumes).
    """
    state = _CHANGEPW_STATE.get(uid)
    if state and time.time() - state.get("ts", 0) > _CHANGEPW_TTL_SEC:
        _CHANGEPW_STATE.pop(uid, None)
        return True
    return False


async def _handle_changepw_document(update: Update, uid: int, doc: Document) -> None:
    """
    Document handler while the user is inside the Change-Password flow.
    Rule: ONLY the account cookie is accepted here —
      • a .txt / .json cookie file, or
      • ONE .zip / .rar archive that contains a single account's cookie.
    Anything else is rejected with a hint (never silently checked as cookies).
    """
    # Multiple files sent at once (album) → only ONE file is allowed
    if getattr(update.message, "media_group_id", None):
        await update.message.reply_text(
            "🔐 <b>Change Password mode</b>\n\n"
            "Send only <b>ONE</b> file — the archive or cookie file "
            "for that single account.\n"
            "<i>Send /cancel to exit this mode.</i>",
            parse_mode=ParseMode.HTML,
        )
        return

    if _changepw_expired(uid):
        await update.message.reply_text(
            "⌛ <b>Change Password session expired.</b>\nStart again with /changepw.",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    kind  = _archive_kind(fname, mime)
    is_cookie_file = (not kind) and (
        any(fname.endswith(ext) for ext in COOKIE_EXTENSIONS)
        or (mime in COOKIE_MIME_TYPES and "octet" not in mime)
    )

    if not kind and not is_cookie_file:
        await update.message.reply_text(
            "🔐 <b>Change Password mode is active.</b>\n\n"
            "Only the account <b>cookie</b> is accepted here:\n"
            "• Paste the cookie text directly\n"
            "• A <code>.txt</code> / <code>.json</code> cookie file\n"
            "• ONE <code>.zip</code> / <code>.rar</code> archive containing a single cookie\n\n"
            "<i>Send /cancel to exit and check cookies normally.</i>",
            parse_mode=ParseMode.HTML,
        )
        return

    status_msg = await update.message.reply_text("⏳ Downloading file…")
    suffix = f".{kind}" if kind else (Path(fname).suffix or ".txt")
    tmp_path = None
    last_error = None
    ...
    try:
        if kind == "zip":
            ...
            entries = read_cookie_texts_from_rar(tmp_path)
        else:
            with open(tmp_path, "rb") as f:
                raw = f.read()
            if _is_binary_content(raw):
                await status_msg.edit_text(
                    "❌ <b>Binary file detected.</b>\nSend a plain-text cookie file.",
                    parse_mode=ParseMode.HTML,
                )
                return
            text = raw.decode("utf-8", errors="replace").strip()
            if text:
                entries = [(doc.file_name or "cookie", text)]
    except zipfile.BadZipFile:
        await status_msg.edit_text(
            "❌ <b>Corrupt or invalid ZIP file.</b>", parse_mode=ParseMode.HTML)
        return
    except Exception as e:
        await status_msg.edit_text(
            f"❌ <b>Could not read the archive.</b>\n<i>{type(e).__name__}: {e}</i>",
            parse_mode=ParseMode.HTML,
        )
        return
    finally:
    ...
    if not entries:
        await status_msg.edit_text(
            "⚠️ <b>No cookie data found.</b>\n"
            "The archive must contain <code>.txt</code> / <code>.json</code> cookie files.",
            parse_mode=ParseMode.HTML,
        )
        return

    # Collect DISTINCT NetflixIds across all extracted entries
    ids: list[str] = []
    for _name, text in entries:
        nid = _extract_netflix_id(text)
        if nid and nid not in ids:
            ids.append(nid)

    if not ids:
        await status_msg.edit_text(
            "❌ <b>No NetflixId found in that file.</b>\n"
            "Required: <code>NetflixId</code> or <code>SecureNetflixId</code> cookie.",
            parse_mode=ParseMode.HTML,
        )
        return
    if len(ids) > 1:
        await status_msg.edit_text(
            f"⚠️ <b>That file contains {len(ids)} different accounts.</b>\n\n"
            "In Change Password mode send <b>ONE</b> account cookie only — "
            "a single cookie file, or an archive holding exactly one cookie.",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    state = _CHANGEPW_STATE.setdefault(uid, {"step": "netflix_id"})
    state["netflix_id"] = ids[0]
    state["step"]       = "old_pw"
    state["ts"]         = time.time()
    await update.message.reply_text(
        f"✅ NetflixId loaded from <b>{doc.file_name or 'file'}</b>.\n\n"
        "Step 2 of 3 — Send the account's <b>current password</b>:\n"
        "<i>Your message will NOT be stored after this step.</i>",
        parse_mode=ParseMode.HTML,
    )
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "Formats: Netscape, JSON, pipe-combo, hit-file, ZIP / RAR.\n\n"
        "📊 <b>Session Stats</b> <i>(since last restart)</i>\n"
        f"{_row('⏱️', 'Uptime', f'<b>{uptime}</b>')}\n"
        f"{_row('📦', 'Total checked', f'<b>{total_checks}</b>')}\n"
        f"{_row('✅', 'Hits', f'<b>{hits}</b>  ({hit_rate}% hit rate)')}\n"
        f"{_row('⏸️', 'On Hold', f'<b>{on_hold}</b>')}\n"
        f"{_row('🔓', 'Free accounts', f'<b>{frees}</b>')}\n"
        f"{_row('❌', 'Invalid/Expired', f'<b>{invalids}</b>')}\n"
        f"{_row('⚠️', 'Errors', f'<b>{errors}</b>')}\n"
        f"{_row('👤', 'Unique users', f'<b>{users}</b>')}\n"
        f"{_row('🔄', 'Active checks', f'<b>{active}</b>')}\n"
        f"{_row('🚀', 'Speed (60s)', f'<b>{cpm} checks/min</b>')}\n\n"
        "⚙️ <b>Your Settings</b>\n"
        f"{_row('📊', 'Output mode', f'<b>{mode_label}</b>')}\n"
        f"{_row('📤', 'Delivery', f'<b>{delivery_label}</b>')}\n\n"
        "📋 <b>Commands</b>\n"
        "/start — Welcome &amp; overview\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Change Password flow — documents are intercepted here ──────────────
    # In changepw mode ONLY the account cookie is accepted: pasted text,
    # a .txt/.json cookie file, or ONE .zip/.rar holding a single cookie.
    if uid and uid in _CHANGEPW_STATE:
        await _handle_changepw_document(update, uid, doc)
        return
    ...
    mime = doc.mime_type or ""
    fname = (doc.file_name or "").lower()
    kind      = _archive_kind(fname, mime)          # "zip" | "rar" | ""
    is_zip    = kind == "zip"
    is_rar    = kind == "rar"
    is_cookie = (not is_zip and not is_rar) and (
        any(fname.endswith(ext) for ext in COOKIE_EXTENSIONS) or mime in COOKIE_MIME_TYPES
    )

    # ── Extension / MIME guard ─────────────────────────────────────────────
    if not kind and not is_cookie:
        ...
        await update.message.reply_text(
            ...
            "• <code>.json</code> — JSON cookie export\n"
            "• <code>.zip</code> / <code>.rar</code> — Multiple cookie files\n\n"
            "<i>Send a cookie file (.txt / .json) or a .zip / .rar archive of them.</i>",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    status_msg = await update.message.reply_text("⏳ Downloading file…")
    suffix  = (".zip" if is_zip else ".rar" if is_rar
               else (Path(fname).suffix or ".txt"))
    tmp_path = None
    last_error = None
    ...
    try:
        if is_zip or is_rar:
            if is_zip:
                ...
                await status_msg.edit_text("📦 Extracting RAR…")
                try:
                    entries = read_cookie_texts_from_rar(tmp_path)
                except Exception as _e:
                    try:
                        import rarfile as _rarfile
                        _is_rar_err = isinstance(_e, _rarfile.Error)
                    except Exception:
                        _is_rar_err = False
                    if _is_rar_err:
                        await status_msg.edit_text(
                            "❌ <b>Corrupt or invalid RAR file.</b>\n\n"
                            "The archive could not be opened. Make sure it is a valid, "
                            "unencrypted RAR archive (RAR4 / RAR5).",
                            parse_mode=ParseMode.HTML,
                        )
                    else:
                        await status_msg.edit_text(
                            f"❌ <b>RAR extraction failed.</b>\n<i>{type(_e).__name__}: {_e}</i>",
                            parse_mode=ParseMode.HTML,
                        )
                    return
                finally:
            ...
            if not entries:
                await status_msg.edit_text(
                    "⚠️ <b>No cookie files found in the archive.</b>\n\n"
                    "Make sure it contains <code>.txt</code> or <code>.json</code> cookie files.",
                    parse_mode=ParseMode.HTML,
                )
                return

            # Validate that at least one file in the archive has Netflix cookies
            ...
            if not _has_netflix_markers(all_text):
                ...
                if wrong:
                    msg = (
                        ...
                        f"This archive contains <b>{wrong}</b> cookies, not Netflix.\n"
                        "Only Netflix cookies are supported."
                    )
                else:
                    msg = (
                        "❌ <b>No Netflix cookies found in this archive.</b>\n\n"
...
async def proxy_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "proxy:toggle":
        ...
        _PROXY_ADD_STATE[uid] = time.time()
        ...
        _PROXY_SOURCE_STATE[uid] = time.time()
        await query.message.reply_text(
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid and uid in _CHANGEPW_STATE:
        if _changepw_expired(uid):
            # Stale session — release the user back to normal cookie checking
            await update.message.reply_text(
                "⌛ <b>Change Password session expired.</b>\n"
                "Continue with a normal cookie check — or start again with /changepw.",
                parse_mode=ParseMode.HTML,
            )
            # fall through — treat this message as a normal cookie check
        else:
    ...
    if not ok:
        ...
        if len(text) < 80 and not any(c in text for c in ("\t", "=", ";")):
            await update.message.reply_text(
                ...
                "Send me a Netflix cookie file (<code>.txt</code>, <code>.json</code>, "
                "<code>.zip</code>, or <code>.rar</code>), "
...
async def _session_watchdog() -> None:
    ...
    while True:
        ...
        # ── Abandoned interactive flows (changepw / proxy add / import) ─────
        try:
            stale_cp = [u for u, st in list(_CHANGEPW_STATE.items())
                        if now - st.get("ts", 0) > _CHANGEPW_TTL_SEC]
            for u in stale_cp:
                _CHANGEPW_STATE.pop(u, None)
        except Exception:
            pass
        try:
            for u in [k for k, ts in list(_PROXY_ADD_STATE.items())
                      if now - ts > _CHANGEPW_TTL_SEC]:
                _PROXY_ADD_STATE.pop(u, None)
            for u in [k for k, ts in list(_PROXY_SOURCE_STATE.items())
                      if now - ts > _CHANGEPW_TTL_SEC]:
                _PROXY_SOURCE_STATE.pop(u, None)
        except Exception:
            pass
    ...
    try:
        await query.message.reply_document(
            ...
            caption=(
                ...
                f"{_row('📋', 'Accounts', f'<b>{len(hits)}</b>')}\n"
                f"{_row('🔑', 'Links', f'<b>{ok_count}</b> / {len(hits)}')}\n\n"
                f"📄 <code>_LOGIN_LINKS_{label.upper().replace(' ', '_')}.txt</code> — account cards + URLs\n"
                f"🗂️ <code>premium/</code> · <code>hits/</code> · <code>on-hold/</code> — cookie files"
            ),
            parse_mode=ParseMode.HTML,
        )
...
async def besthits_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Score and pick top 5 — ACTIVE hits only.  On-hold accounts are
        # never ranked as "best" (they are not usable hits).
        active_pool = [x for x in hits_list if x[0].get("status") == "hit"]
        if not active_pool:
            await query.message.reply_text(
                f"⏸️ <b>No active hits.</b>\n"
                f"All {len(hits_list)} account(s) in this batch are ON-HOLD "
                f"(payment issue) — no premium hits to rank.",
                parse_mode=ParseMode.HTML,
            )
            return
        try:
            sorted_hits = sorted(active_pool, key=lambda x: _score_account(x[0]), reverse=True)
        except Exception:
            sorted_hits = active_pool[:]
        top_n = sorted_hits[:5]
        ...
        full_text = (
            f"🏆 <b>Best Hits</b>  •  Top {len(top_n)} active of {len(hits_list)} found\n"
            f"<i>Ranked: Active → plan tier → billing → member age</i>\n\n" + txt
        )
        if len(full_text) > 4090:
            full_text = full_text[:4087] + "…"
...
async def process_cookie_sets(
    ...
    for batch_start in range(0, total, BULK_CONCURRENCY):
        if session_id in _CANCEL_SESSIONS:
            ...
            if hits_list:
                await update.message.reply_text(
                    ...
                    f"{_row('✅', 'Hits (Active)', f'<b>{hits}</b>')}\n"
                    f"{_row('⏸️', 'On Hold', f'<b>{on_hold}</b>')}\n"
                    f"{_row('🔓', 'Free (No Sub)', f'<b>{frees}</b>')}\n"
                    f"{_row('❌', 'Invalid', f'<b>{invalids}</b>')}\n"
                    f"{_row('⚠️', 'Errors', f'<b>{errors}</b>')}\n\n"
                    f"{_row('📦', 'Checked', f'<b>{batch_start}</b> / {total}')}",
                    parse_mode=ParseMode.HTML,
                )
        ...
        for _off, ((src, cs, raw), result) in enumerate(zip(batch, batch_results)):
            if isinstance(result, Exception):
                result = {"status": "error", "message": str(result)}

            status = result.get("status", "error")
            idx    = batch_start + _off + 1
    ...
    if is_bulk:
        # Active vs on-hold split — on-hold accounts are NOT hits and must
        # never be counted in the Premium number (their plan name may still
        # say "Premium" but the account is payment-held).
        active_hits_list = [(r, s, w) for r, s, w in hits_list if r.get("status") == "hit"]
        onhold_hits_list = [(r, s, w) for r, s, w in hits_list if r.get("status") == "on_hold"]
        premium_count = sum(1 for r, _, _ in active_hits_list
                            if "premium" in (r.get("plan_name") or "").lower())
        _hold_note = ""
        if onhold_hits_list:
            _held_prem = sum(1 for r, _, _ in onhold_hits_list
                             if "premium" in (r.get("plan_name") or "").lower())
            _hold_note = (
                f"\n<i>ℹ️ {_held_prem} on-hold account(s) have premium plans, "
                f"but they are NOT hits (payment issue).</i>"
                if _held_prem else
                f"\n<i>ℹ️ On-hold accounts are not counted as hits.</i>"
            )
        try:
            await status_msg.edit_text(
        ...
        await update.message.reply_text(
            ...
            f"{_row('✅', 'Hits (Active)', f'<b>{hits}</b>  ·  🌟 Premium: {premium_count}')}\n"
            f"{_row('⏸️', 'On Hold', f'<b>{on_hold}</b> (not hits)')}\n"
            f"{_row('🔓', 'Free (No Sub)', f'<b>{frees}</b>')}\n"
            f"{_row('❌', 'Invalid/Expired', f'<b>{invalids}</b>')}\n"
            f"{_row('⚠️', 'Errors', f'<b>{errors}</b>')}\n\n"
            f"{_row('📦', 'Total Checked', f'<b>{total}</b>')}\n"
            f"{_row('⏱️', 'Time', f'<b>{time_str}</b>')}\n"
            f"{_row('🚀', 'Speed', f'<b>{speed:.1f} acc/min</b>')}"
            f"{_hold_note}",
            parse_mode=ParseMode.HTML,
        )
        ...
        if hits_list:
            ...
            if delivery == "cards":
                ...
                # Only ACTIVE hits compete for the 🏆 — an on-hold account
                # (even premium plan) can never be the "best" account.
                try:
                    active_sorted = sorted(
                        active_hits_list,
                        key=lambda x: _score_account(x[0]), reverse=True)
                    if active_sorted:
                        best_result, best_src, _ = active_sorted[0]
                        ...
                        full_best = (
                            ...
                            f"<i>top ACTIVE account — plan · billing · member age</i>\n"
                            f"<i>(on-hold accounts excluded from ranking)</i>\n\n"
                            + best_txt
                        )
                    ...
                    elif onhold_hits_list:
                        await update.message.reply_text(
                            f"⏸️ <b>No active hits in this batch.</b>\n\n"
                            f"All {len(onhold_hits_list)} found account(s) are ON-HOLD "
                            f"(payment issue) — no premium hits found.\n"
                            f"<i>Saved in the</i> <code>On Hold/</code> <i>folder of the ZIP.</i>",
                            parse_mode=ParseMode.HTML,
                        )
