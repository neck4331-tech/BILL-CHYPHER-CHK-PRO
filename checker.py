...
def parse_json_cookies(text: str) -> dict:
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            # Browser-extension exports wrap the cookie list under "cookies":
            #   {"cookies": [{"name": "NetflixId", "value": "..."}, ...]}
            # Flatten every known wrapper shape into a flat {name: value} dict,
            # because check_cookie expects top-level NetflixId/SecureNetflixId.
            ...
            if isinstance(inner, list):
                ...
                cookies.update(inner)
            # Some bots export a raw header string: {"cookie": "NetflixId=...; ..."}
            if not cookies and isinstance(data.get("cookie"), str):
                cookies = parse_header_cookies(data["cookie"])
            if not cookies:
                cookies = data
    except json.JSONDecodeError:
        pass
...
def _extract_growth_hold(html: str) -> dict:
    """
    Detect hold state from Netflix's modern GraphQL (Apollo) data — the
    ROOT_QUERY.growthAccount block. This is what Netflix serves NOW; the old
    Falcon-format MemberPlan.isOnHold field is absent for these accounts.

    Example real payload:
      "growthHoldMetadata":{"__typename":"GrowthHoldMetadata",
        "isUserOnHold":true,
        "retryEligibility":"NOT_ELIGIBLE",
        "serviceEndReason":"SERVICE_END_PAYMENT_FAILURE"}

    Returns: is_on_hold (bool), has_feature_only_hold (bool),
             service_end_reason (str), retry_eligibility (str)
    100% language-agnostic — pure JSON keys/enum values, no UI text.
    """
    result: dict = {}
    m = re.search(r'"isUserOnHold"\s*:\s*(true|false)', html, re.IGNORECASE)
    if m:
        result["is_on_hold"] = m.group(1).lower() == "true"
    m = re.search(r'"serviceEndReason"\s*:\s*"([^"]+)"', html)
    if m:
        result["service_end_reason"] = m.group(1)
    m = re.search(r'"hasFeatureOnlyHold"\s*:\s*(true|false)', html, re.IGNORECASE)
    if m:
        result["has_feature_only_hold"] = m.group(1).lower() == "true"
    m = re.search(r'"retryEligibility"\s*:\s*"([^"]+)"', html)
    if m:
        result["retry_eligibility"] = m.group(1)
    return result


# Human-readable labels for GraphQL serviceEndReason enum values
SERVICE_END_REASON_MAP = {
    "SERVICE_END_PAYMENT_FAILURE": "Payment failed — service ending soon",
    "SERVICE_END_CANCELLED":       "Account cancelled",
    "SERVICE_END_TRIAL_ENDED":     "Trial ended — payment required",
    "SERVICE_END_PAYMENT_DECLINED":"Payment declined by bank",
    "SERVICE_END_UNSUCCESSFUL_CHARGE": "Charge unsuccessful — update payment",
}

# Netflix GraphQL planId → canonical plan (planId is stable across locales,
# unlike the localized plan name e.g. "Temel" = Basic in Turkish)
GQL_PLAN_ID_MAP = {
    "4000": {"name": "Mobile",  "quality": "480p",   "streams": 1},
    "4001": {"name": "Basic",   "quality": "480p",   "streams": 1},
    "4002": {"name": "Standard","quality": "1080p",  "streams": 2},
    "4003": {"name": "Premium", "quality": "4K+HDR", "streams": 4},
}

# Localized plan names → canonical plan (covers pages Netflix serves in
# non-English locales where currentPlan.plan.name is translated)
LOCALIZED_PLAN_MAP = {
    # Basic
    "temel": "basic", "basico": "basic", "básico": "basic", "basique": "basic",
    "einfach": "basic", "basis": "basic", "essenziale": "basic", "ベーシック": "basic",
    "基础": "basic", "기본": "basic", "базовый": "basic", "أساسي": "basic",
    # Standard
    "standart": "standard", "padrão": "standard", "estándar": "standard",
    "standard mit werbung": "standard with ads", "con anuncios": "standard with ads",
    "avec publicités": "standard with ads", "reklamlı standart": "standard with ads",
    "스탠다드": "standard", "スタンダード": "standard", "标准": "standard",
    # Premium
    "prémium": "premium", "premium plus": "premium",
}


def _extract_growth_account(html: str) -> dict:
    """
    Extract plan + billing data from the GraphQL ROOT_QUERY.growthAccount
    block (Apollo cache format). Complements the Falcon-format extractors
    which miss accounts served only in the new format.

    Returns any of: plan_name, plan_id, canonical_plan, next_billing,
                    member_since, is_on_hold, service_end_reason
    """
    result: dict = {}

    # planId + localized plan name from GrowthPlan
    m = re.search(
        r'"currentPlan"\s*:\s*\{"__typename"\s*:\s*"GrowthCustomerPlanPackage"\s*,\s*"plan"\s*:\s*\{[^}]*?"planId"\s*:\s*"(\d+)"',
        html,
    )
    if m:
        result["plan_id"] = m.group(1)
    m = re.search(
        r'"currentPlan"\s*:\s*\{"__typename"\s*:\s*"GrowthCustomerPlanPackage".*?"attributes"\s*:\s*\{"__typename"\s*:\s*"GrowthPlanAttributes"[^}]*\}\s*,\s*"name"\s*:\s*"([^"]+)"',
        html,
    )
    if m:
        result["plan_name"] = m.group(1)

    # Resolve canonical plan via planId first (locale-proof), then localized name
    canonical = None
    if result.get("plan_id") and result["plan_id"] in GQL_PLAN_ID_MAP:
        canonical = GQL_PLAN_ID_MAP[result["plan_id"]]
    elif result.get("plan_name"):
        pname = result["plan_name"].lower()
        if pname in LOCALIZED_PLAN_MAP:
            key = LOCALIZED_PLAN_MAP[pname]
            for pk, info in PLAN_MAP.items():
                if pk == key or info["name"].lower() == key:
                    canonical = info
                    break
        else:
            for key, info in PLAN_MAP.items():
                if key in pname:
                    ...
                    break
    if canonical:
        result["canonical_plan"] = canonical

    # Next billing date — GraphQL GrowthNextBillingDate
    html_decoded = _decode_js_hex(html)
    m = re.search(
        r'"nextBillingDate"\s*:\s*\{"__typename"\s*:\s*"GrowthNextBillingDate"\s*,\s*"date"\s*:\s*"([^"]+)"',
        html_decoded,
    )
    if m:
        result["next_billing"] = _parse_date(m.group(1))

    # Member since — ISO timestamp in GraphQL block
    m = re.search(r'"memberSince"\s*:\s*"(\d{4}-\d{2}-\d{2})', html_decoded)
    if m:
        result["member_since"] = _parse_date(m.group(1))
...
def _deep_search_ctx(obj, depth: int = 0) -> list[tuple[str, str]]:
    ...
    if isinstance(obj, dict):
        for key, val in obj.items():
            ...
            # ── isUserOnHold true from GraphQL growthHoldMetadata ──────
            elif key == "isUserOnHold" and val is True:
                hits.append(("gql_on_hold", "Account on hold (payment failed)"))

            # ── GraphQL serviceEndReason enum ──────────────────────────
            elif key == "serviceEndReason" and isinstance(val, str) and val in SERVICE_END_REASON_MAP:
                hits.append(("gql_service_end", SERVICE_END_REASON_MAP[val]))
...
def _detect_account_issues(html: str) -> list[str]:
    ...
    for flag, label in {
        ...
        "isUserOnHold":       "Account on hold (payment failed)",
    ...
    # GraphQL serviceEndReason (modern accounts, any locale)
    ser_m = re.search(r'"serviceEndReason"\s*:\s*"([^"]+)"', html_decoded)
    if ser_m:
        _ser = ser_m.group(1)
        if _ser in SERVICE_END_REASON_MAP:
            _add(SERVICE_END_REASON_MAP[_ser])

    # Banner text fallbacks (English pages only — JSON keys above are
    # locale-proof; these catch pages where reactContext is stripped)
    _banner_patterns = [
        (r'payment\s+transaction\s+failed',
         "Payment failed — update payment method"),
        (r'unable\s+to\s+process\s+your\s+payment',
         "Payment could not be processed — update payment method"),
        (r'update\s+your\s+payment\s+information',
         "Payment update required"),
    ]
    _html_text = re.sub(r'\\u([0-9a-fA-F]{4})', ' ', html_decoded).lower()
    for pat, label in _banner_patterns:
        if re.search(pat, _html_text):
            _add(label)
...
def check_cookie(cookie_text: str, generate_token: bool = True, bulk_mode: bool = False) -> dict:
    ...
    try:
        ...
        # GraphQL (Apollo) data — modern Netflix accounts serve plan, billing
        # and hold state here; the Falcon-format MemberPlan block is absent.
        growth_info = _extract_growth_account(account_html) or _extract_growth_account(browse_html)
        growth_hold = _extract_growth_hold(account_html) or _extract_growth_hold(browse_html)
        ...
        if not plan_name and growth_info.get("plan_name"):
            plan_name = growth_info["plan_name"]
        # Locale-proof plan resolution: planId / localized-name mapping fills
        # quality + streams + canonical English name for translated pages.
        if growth_info.get("canonical_plan"):
            canon = growth_info["canonical_plan"]
            plan_name   = canon["name"]
            quality     = quality     or canon["quality"]
            max_streams = max_streams or str(canon["streams"])
        ...
        # Hold flag: Falcon MemberPlan.isOnHold OR GraphQL isUserOnHold
        result["is_on_hold"]       = bool(plan_info.get("is_on_hold", False)
                                          or growth_hold.get("is_on_hold", False))
        result["num_extra_members"]= plan_info.get("num_extra_members", 0)
        result["plan_id"]          = growth_info.get("plan_id", "")
        result["service_end_reason"] = growth_hold.get("service_end_reason", "")
        result["retry_eligibility"]  = growth_hold.get("retry_eligibility", "")

        # Payment info is needed early: partner-billed bundle accounts need
        # different hold handling (see bundle exemption below). Computed once
        # here and reused later for the payment/card fields.
        pay_info = _extract_payment_info(account_html) or _extract_payment_info(browse_html)

        # ── Partner-billed bundle (ISP / telco) hold exemption ────────────────
        # Third-party bundle accounts (e.g. Infinitum MX, T-Mobile US, Sky EU)
        # routinely carry growthHoldMetadata.isUserOnHold:true with
        # serviceEndReason SERVICE_END_BUNDLE_SUSPEND and retryEligibility
        # NOT_ELIGIBLE even while FULLY ACTIVE. Netflix never bills them
        # directly — the partner does — so the payment-retry flow does not
        # apply and that metadata is boilerplate for them. Treating it as a
        # payment hold flagged working accounts as ON-HOLD (false positive).
        #
        # The exemption below only applies when ALL of these hold:
        #   1. bundle/partner billing detected
        #   2. membershipStatus is CURRENT (Netflix's authoritative state)
        #   3. serviceEndReason is NOT a real failure reason (not in map)
        #   4. zero hard payment-failure evidence in the HTML
        #   5. no Falcon MemberPlan.isOnHold (plan-level hold is always real)
        # A genuinely held account fails at least one condition → still on_hold.
        _compact_html = re.sub(r'\s+', '', _decode_js_hex(account_html)).lower()
        _bundle_billing = bool(pay_info.get("is_third_party")) or (
            '"__typename":"growthbundlepaymentmethod"' in _compact_html)
        _ms_current = bool(ms_status) and ms_status.startswith("CURRENT")
        _no_real_reason = result.get("service_end_reason", "") not in SERVICE_END_REASON_MAP
        _pay_fail_evidence = (
            "paymenttransactionfailed" in _compact_html
            or "unabletoprocessyourpayment" in _compact_html)
        _falcon_hold = bool(plan_info.get("is_on_hold", False))
        _bundle_exempt = bool(
            _bundle_billing and _ms_current and _no_real_reason
            and not _pay_fail_evidence and not _falcon_hold)

        # ── Backfill plan-level on-hold flag into issues + status ─────────────
        # Sources: MemberPlan.isOnHold (Falcon) and growthHoldMetadata.isUserOnHold
        # (GraphQL). Both are reliable even when membershipStatus is still
        # "CURRENT_MEMBER" — that is the grace-period state where the payment
        # already failed but Netflix hasn't cut service yet.
        # EXCEPTION: partner-billed bundle accounts (see exemption above) —
        # their isUserOnHold metadata is boilerplate, not a real payment hold.
        if result["is_on_hold"] and result["status"] == "hit":
            if _bundle_exempt:
                # Active bundle account — drop the false hold flag and any
                # hold-flavored issues _detect_account_issues picked up.
                result["is_on_hold"] = False
                result["account_issues"] = [
                    i for i in (result.get("account_issues") or [])
                    if not any(t in i.lower() for t in (
                        "on hold", "hold", "payment failed", "payment due",
                        "payment issue", "update payment"))
                ]
            else:
                result["status"] = "on_hold"
                existing = result.get("account_issues") or []
                _reason = result.get("service_end_reason", "")
                _hold_msg = SERVICE_END_REASON_MAP.get(
                    _reason, "Account on hold (payment failed)")
                if not any("on hold" in i.lower() or "payment failed" in i.lower() for i in existing):
                    result["account_issues"] = list(existing) + [_hold_msg]
        ...
        # ── Member since ──────────────────────────────────────────────────────
        member_since = None
        if not member_since and growth_info.get("member_since"):
            member_since = growth_info["member_since"]
        if userinfo:
        ...
        next_billing = (plan_info.get("next_billing")
                        or growth_info.get("next_billing")
        ...
        # ── Payment ───────────────────────────────────────────────────────────
        # pay_info was already extracted above (needed for the bundle hold
        # exemption) — reuse it instead of parsing the HTML a second time.
        ...
        # ── On-hold final check ───────────────────────────────────────────────
        # Generic hold strings/flags are skipped for partner-billed bundle
        # accounts (_bundle_exempt) — for them only hard payment-failure
        # evidence (_pay_fail_evidence) can mark a hold.
        _html_compact = re.sub(r'\s+', '', account_html).lower()
        if result["status"] == "hit":
            _generic_hold = (
                result["is_on_hold"]
                or "on hold" in _html_compact
                or "payment hold" in _html_compact
                or '"isuseronhold":true' in _html_compact)
            if _pay_fail_evidence or (_generic_hold and not _bundle_exempt):
                result["status"] = "on_hold"
                if not any("hold" in i.lower() or "payment" in i.lower()
                           for i in (result.get("account_issues") or [])):
                    result["account_issues"] = list(result.get("account_issues") or []) + [
                        SERVICE_END_REASON_MAP.get(result.get("service_end_reason", ""),
                                                   "Account on hold (payment failed)")]
